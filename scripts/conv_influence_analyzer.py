...
def analyze_conv_channel_influence(args):
        ...
        if isinstance(module, nn.Conv2d):
            ...
            def make_forward_hook(layer_name):
                def forward_hook_fn(module, input_tensors, output_tensor):
                    ...
                    # 对于模型的初始输入，我们会在数据加载循环中手动设置 requires_grad=True
                    # 对于中间层，如果它们是计算图的一部分且需要梯度（因为它们是损失函数路径上的），
                    # PyTorch 会自动设置 requires_grad=True
                    ...
                    # else:
                    #     # 如果有层输入 requires_grad 为 False，这通常意味着它不在损失的计算路径上
                    #     # 或者没有为原始输入设置 requires_grad
                    #     print(f"警告: 卷积层 '{layer_name}' 的输入张量 requires_grad 为 False，无法获取梯度。")
    ...
    # 在这个外部的 no_grad() 块中，我们仍然可以通过手动设置输入张量的 requires_grad=True 来计算梯度
    # 这确保了模型在评估模式下行为，但我们仍然可以进行梯度分析
    with torch.no_grad(): 
        for i, (inputs, labels) in enumerate(testloader):
            ...
            # 使用交叉熵损失作为梯度计算的起点。如果你分析的目标是其他输出（例如某个特定类别的得分），
            # 可以直接对 outputs[:, target_class_idx].sum() 调用 .backward()
